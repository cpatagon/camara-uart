...
# -*- coding: utf-8 -*-
"""
Cliente UART Robusto - Con protocolo ACK bidireccional
"""
...
class UARTPhotoClientRobust:
    ...
    def connect(self):
        ...
        try:
            ...
            logging.info(f"✅ Cliente Robusto: {self.port} @ {self.baudrate} (rtscts={self.rtscts}, xonxoff={self.xonxoff})")
            return True
    ...
    def _read_exact_robust(self, nbytes, inactivity_timeout=45, chunk_size=4096):
        """Lectura exacta robusta con manejo de retransmisiones"""
        remaining = nbytes
            ...
            if chunk:
                ...
                if nbytes >= 10000:
                    ...
                    if progress - last_progress >= 10:
                        logging.info(f"📊 Progreso robusto: {got}/{nbytes} bytes ({progress}%)")
    ...
    def receive_image_robust(self, expected_size, save_path=None, enable_ack=True):
        """Recepción robusta con protocolo ACK completo"""
        try:
            logging.info("📥 Iniciando recepción ROBUSTA...")
            ...
            # 4. Recepción principal robusta
            self.received_data = bytearray()
            success = self._read_exact_robust(transmitted_size, inactivity_timeout=60)
            ...
            if not save_path:
                ...
                save_path = f"imagen_robust_{timestamp}.jpg"
            ...
            if success_final:
                logging.info("🎉 RECEPCIÓN ROBUSTA EXITOSA")
            else:
            ...
            logging.error(f"❌ Error en recepción robusta: {e}")
            if enable_ack:
                try:
    ...
    def close(self):
        ...
        try:
            if self.ser and self.ser.is_open:
                ...
                self.ser.reset_output_buffer()
                self.ser.close()
                logging.info("🔌 Cliente robusto cerrado")
        except Exception as e:
            logging.debug(f"Error cerrando cliente: {e}")

def main():
    parser = argparse.ArgumentParser(description='Cliente UART Robusto con protocolo ACK')
    ...
    print("=" * 70)
    print("Cliente UART ROBUSTO con protocolo ACK bidireccional")
    print("=" * 70)

    client = UARTPhotoClientRobust(
        port=args.port,
        baudrate=args.baudrate,
        timeout=10,  # Timeout más largo para robustez
        rtscts=args.rtscts,
    ...
    try:
        ...
        # Recepción robusta con ACK
        success = client.receive_image_robust(expected_size, args.output, enable_ack=enable_ack)

        if success:
            logging.info("=" * 70)
            logging.info("🎉 ¡PROCESO ROBUSTO COMPLETADO CON ÉXITO!")
            logging.info("=" * 70)
        else:
            logging.error("=" * 70)
            logging.error("❌ ERROR EN EL PROCESO ROBUSTO")
            logging.error("=" * 70)

    except KeyboardInterrupt:
        logging.info("\n🛑 Cliente robusto detenido por usuario")
    except Exception as e:
